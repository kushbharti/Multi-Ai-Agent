    ...
    <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {messages.length == 0 || !selectedconversation ? (
        <div className="h-full flex flex-col items-center justify-center gap-4 text-center">
          ...
          <div>
            {["Build Netflix clone", "Explain Redis ", "Build Dashboard"].map(
              (item) => (
                <button className="text-[12px] text-slate-400 bg-white/4 border ml-3 border-white/7 px-3 py-1.5 rounded-lg hover:bg-white/8 hover:text-slate-200 transition-colors duration-150 cursor-pointer">
                  {item}
                </button>
              ),
            )}
